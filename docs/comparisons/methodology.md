---
title: Methodology
layout: default
parent: Comparisons
---

# Methodology Frameworks Comparison

∵ RCR Regis ∴

Comparing 9 frameworks in the **methodology** category.

## Overview

| Framework | Version | SOLID | Production | Best For |
|-----------|---------|-------|------------|----------|
| [BMAD Method (Build More, Architect Dreams)](../frameworks/bmad-method.md) | 6.0.0-alpha.12 | 4.3/5.0 | 87/100 | - |
| [claude-code-dev-kit](../frameworks/claude-code-dev-kit.md) | 2.1.0 | 4.2/5.0 | 64/100 | - |
| [RIPER-5](../frameworks/riper-5.md) | 2025.11 | 4.2/5.0 | 72/100 | - |
| [taskmaster-mcp](../frameworks/taskmaster-mcp.md) | 0.15.0 | 4.2/5.0 | 74/100 | - |
| [SuperClaude](../frameworks/superclaude.md) | 4.1.9 | 4.0/5.0 | 78/100 | - |
| [Ab Method](../frameworks/ab-method.md) | 2.3.0 | 3.9/5.0 | 74/100 | - |
| [claude-code-heavy](../frameworks/claude-code-heavy.md) | 1.0.0 | 3.8/5.0 | 52/100 | - |
| [ContextKit](../frameworks/contextkit.md) | 1.0.0 | 3.8/5.0 | 75/100 | - |
| [Simone](../frameworks/simone.md) | 0.4.0-mcp | 3.6/5.0 | 63/100 | - |

## Rankings

### By SOLID Principles

1. **BMAD Method (Build More, Architect Dreams)** - 4.3/5.0
2. **claude-code-dev-kit** - 4.2/5.0
3. **RIPER-5** - 4.2/5.0
4. **taskmaster-mcp** - 4.2/5.0
5. **SuperClaude** - 4.0/5.0
6. **Ab Method** - 3.9/5.0
7. **claude-code-heavy** - 3.8/5.0
8. **ContextKit** - 3.8/5.0
9. **Simone** - 3.6/5.0

### By Production Readiness

1. **BMAD Method (Build More, Architect Dreams)** - 87/100
2. **SuperClaude** - 78/100
3. **ContextKit** - 75/100
4. **Ab Method** - 74/100
5. **taskmaster-mcp** - 74/100
6. **RIPER-5** - 72/100
7. **claude-code-dev-kit** - 64/100
8. **Simone** - 63/100
9. **claude-code-heavy** - 52/100

## Key Differentiators

### Ab Method




### BMAD Method (Build More, Architect Dreams)




### claude-code-dev-kit




### claude-code-heavy




### ContextKit




### RIPER-5




### Simone




### SuperClaude




### taskmaster-mcp





## Decision Matrix

| Need | Recommended |
|------|-------------|
| Best Architecture |  |
| Production Ready |  |
| Balanced Choice |  |

---

## Navigation

- [← All Comparisons](index.md)
- [Full Synthesis](../synthesis.md)
- [Search](../search.html)